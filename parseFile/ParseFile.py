...
def parse_txtfile(in_file):
    ...
    """
    vfile = open(in_file, "r", encoding='UTF-8')        # python3 open文件带有中文需要加上encoding='UTF-8'
    new_list = []
...
def line_parse(line):
    ...
    # print line_list
    length_oneline_api = int((len(line_list) - 1) / 2)          # python3 len(line_list) - 1) / 2 返回double型：EG：1.0，需要int()处理
...
def write_new_txt(txt_list, new_file):
    ...
    :return:
    """
    with open(new_file, "a+",  encoding='UTF-8') as f:  # 以追加的方式
        f.truncate()                 # 写入前清空文件
        for li in txt_list:
