...
def record_data(url, method, header, request, response):
    ...
    with open(record_file, 'ab+') as f:
        ...
        # f.write(bytes(json.dumps(flow, indent=4, cls=MyEncoder), encoding="utf-8"))
        f.write(',')
        f.write('\r\n')
...
def main():
    try:
        ...
        child =subprocess.Popen("mitmdump --no-ssl-insecure -w out", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
